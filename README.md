...
| `alerting.threema-gateway.sender-id`         | Threema ID of the sender (from Gateway API identity)                                        | Required `""`               |
| `alerting.threema-gateway.recipient`         | Recipient in format `<type>:<identifier>`, where type is one of `threema`, `phone`, `email` | Required `""`               |
